---
publish: true
title: Changelog 1
created: 2026-07-23T08:25:41.594+02:00
modified: 2026-09-23T08:59:02.876+02:00
published: 2026-09-23T08:59:02.876+02:00
tags:
  - "#Changelog"
  - "#5e"
dateitags:
  - "#Changelog"
  - "#5e"
status: ✅
datum: 2026-07-29
aenderungen:
  - "Heritages: All"
  - "Destinies: Devotion, Fire, Metamorphosis"
  - "Classes: Adept, Fighter, Paladin"
  - "Changed Spells: Many Arcane, Divine and Primal Cantrips"
  - "Added Spells: Added Cantrips to Arcane, Divine, Primal, Bard and Blood Hunter list"
---

# 5e - Changelog 1

### Heritages

#### All Heritages

- If a Feature of your Heritage would force a Creature to make a Save (e.g. the Drakari's Breath Weapon), it no longer specifies how the DC for that Feature is calculated and instead uses your Maneuver Save DC or your Spell save DC. Some Features (like the Drakari's Breath Weapon) can use either, some use one of them.

#### Dwarf

- **<u>Toughest of them all</u>** -> **<u>Slow and Steady</u>**: Your movement speed cannot be reduced by difficult terrain (The GM might rule, that in specific instances this Feature doesn't work as written).
  - **Formerly:** Your movement speed cannot be reduced.

### Destinies

#### Devotion Destiny

- **<u>Inspiration Feature: Selfless Aid:</u>** You can now take the Help Action as a Bonus Action as part of the Feature.

#### Fire Destiny

- **Fulfillment Feature: Blazing Soul:** Increased duration for the Ignite Feature, Scaling damage (PB x d4 instead of 2d4).

#### Metamorphosis Destiny

- **<u>Inspiration Feature: Unearthly Diplomacy:</u>** Creatures of a CR lower than your level might by charmed by this Feature for the duration.

### Classes

#### Adept

- **<u>Level 2: Exertion Focus</u>**: The Saving throw now equals your Maneuver Save DC and is no longer based on WIS.

#### Fighter

- **<u>Fighting Style: Great Weapon Fighting</u>**: You no longer reroll 1s or 2s on the damage die, instead you treat 1s and 2s as a 3.
- **<u>Brute Subclass:</u>** Removed, since for some reason 3/6 subclass Features were the exact same -> no real subclass

#### Paladin

- **<u>Divine Lesson: Exemplary</u>**: You now gain an expertise die on all acrobatics and athletics checks, not only to climb, jump, run, and swim.

### Spells

#### Changed Spells

**Bardcraft:** Now gives Advantage on Performance checks.
**Contagious Thoughts:** Increased damage from 1d6 to 1d8.
**Druidcraft:** Changed out the text to the 2024PHB Text. There might be changes.
**Friends:** Added the Spell to the Arcane list (in addition to bard).
**Frostbite:** Increased damage from 1d6 to 1d8.
**Gust:** Added the Spell to the Arcane list (in addition to primal). Added the option to push a target Prone. Added scaling range with higher levels.
**Hemokinesis:** Added the Spell to the Arcane list (in addition to Blood Hunter).
**Infestation:** Increased damage from 1d6 to 2d6.
**Magic Stone:** Renamed the Spell to "Magic Fling". Changed damage formula from 1d6 + Spellcasting ability to 1d4 + Spellcasting ability. Changed "pebble" to "tiny objects". The Attack with the item now counts as an Attack action.
**Minor Illusion:** Changed out the text to the 2024PHB Text. There might be changes.
**Mold Earth:** Added the Spell to the Arcane list (in addition to primal).
**Pestilence:** Increased damage from 1d4 to 1d6.
**Poison Spray:** Changed Spellschool from Necromancy to Evocation.
**Prestidigitation:** Changed out the text to the 2024PHB Text. There might be changes.
**Rend Flesh:** Added the Spell to the Arcane list (in addition to Blood Hunter).
**Sacred Flame:** Increased damage from 1d8 to 1d10.
**Shape Water:** Added the Spell to the Arcane list (in addition to primal).
**Shocking Grasp:** Increased damage from 1d8 to 1d10.
**Spare the Dying:** Casting time changed from Action to Bonus Action.
**Sword Burst:** Increased damage from 1d6 to 1d8.
**Thaumaturgy:** Changed out the text to the 2024PHB Text. There might be changes.
**Thunderclap:** Increased damage from 1d6 to 2d4.
**Vicious Mockery:** Increased damage from 1d4 to 1d6.
**Void Maw:** Changed Spellschool from Conjuration to Necromancy. Increased damage from 1d6 to 1d8.
**Word of Radiance:** Increased damage from 1d8 to 1d10.

#### New Spells

**Acid Burn:** Arcane and Blood Hunter Spell list.
**Bacterial Barrage:** Arcane and Primal Spell list.
**Blood Bolt:** Arcane and Blood Hunter Spell list.
**Calling Card:** Arcane and Bard Spell list.
**Flare:** Arcane and Divine Spell list.
**Green-Flame Blade:** Arcane and Blood Hunter Spell list.
**Poison Needle:** Arcane and Primal Spell list.
**Spark:** Arcane and Primal Spell list.
